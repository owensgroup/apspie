...
struct fakeCsr {
    ...
    template<typename VertexId>
    void copyCsr( const int node, const int edge, const VertexId *rowPtr, const VertexId *colInd ) {
        m = node;
        std::copy(rowPtr[0],rowPtr[m],h_rowPtrA);
        std::copy(colInd[0],colInd[edge],h_colIndA);
    }
    ...
    VertexId                                stop)
{
    ...
    while (!frontier.empty()) {
        ...
        int edges_begin = graph.h_rowPtrA[dequeued_node];
        int edges_end = graph.h_rowPtrA[dequeued_node + 1];

        for (int edge = edges_begin; edge < edges_end; ++edge) {
            //Lookup neighbor and enqueue if undiscovered
            VertexId neighbor = graph.h_colIndA[edge];
...
int bfsCPU( const int src, const fakeCsr<int> &graph, int *h_bfsResultCPU, const int stop ) {
...
int main(int argc, char**argv) {
    ...
    fakeCsr<int> csr;
    csr.template copyCsr<int>( m, edge, h_rowPtrA, h_colIndA );

    int depth = 1000;
    depth = bfsCPU( 0, &csr, h_bfsResultCPU, depth );
