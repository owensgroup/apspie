...
if [ "$ARCH" = "GEN_SM15" ] ; then
    ...
    aprun -n3 -N1 ./test dataset/small/test_ac.mtx -multi 3
fi

if [ "$ARCH" = "GEN_SM20" ] ; then
    aprun -n 1 ./test dataset/small/test_cc.mtx
    aprun -n 1 ./test dataset/small/test_bc.mtx
    aprun -n 1 ./test dataset/small/test_pr.mtx
    aprun -n 1 ./test dataset/small/chesapeake.mtx
fi
...
do
    if [ "$ARCH" = "GEN_SM35" ] ; then
        ...
        if [ "$ARCH" = "GEN_SM40" ] ; then
            aprun -n 1 ./test dataset/large/soc-LiveJournal1/soc-LiveJournal1.mtx
            aprun -n 1 ./test dataset/large/kron_g500-logn21/kron_g500-logn21.mtx
            break
        fi
    fi
done
...
do
    if [ "$ARCH" = "GEN_SM45" ] ; then
        aprun -n 1 test /data/PPOPP15/$i.mtx
    fi
done 
